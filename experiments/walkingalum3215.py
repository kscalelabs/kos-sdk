...
class BipedController:
    """
    Bipedal walking logic with hip roll for stepping and feet parallel to the ground.
    """
    ...
    def control_foot_position(self, x, y, h, side):
        ...
        # Hip roll for weight shifting
        self.K1[side] = math.atan2(y, h) if abs(h) >= 1e-8 else 0.0

        # Ankle pitch angle to keep the foot parallel to the ground
        self.A0[side] = -(self.K0[side] + self.H[side])
    ...
    def update_gait(self):
        ...
        if self.gait_phase == 0:
            ...
            # Update forward offset for both feet
            self.forward_offset[self.stance_foot_index] = self.step_length * fraction
            # Swing foot moves from behind to in front
            swing_foot_index = self.stance_foot_index ^ 1
            self.forward_offset[swing_foot_index] = -self.step_length * (1 - fraction)

            # Lift the swing foot with more height
