...
class GUIProcess(Process):
    ...
    def __init__(self, skill_name: str, command_queue: Queue, position_queue: Queue) -> None:
        super().__init__()
    ...
    def run(self) -> None:
        ...
        for delay in [0.5, 1.0, 2.0]:
            ttk.Button(delays_frame, text=f"{delay}s", command=lambda d=delay: delay_var.set(str(d))).pack(  # type: ignore
                side=tk.LEFT, padx=5
            )
